...
let r = await findSources(root, ['Canh quay mo dau', 'Close Up San Pham', 'khong-co-that']);
...
r = await findSources(root, ['canh quay mo dao']);
...
r = await findSources(root, ['vo-02']);
...
r = await findSources(root, ['vo-01']);
...
r = await findSources(root, ['Higg 33']);
...
r = await findSources(root, ['Higg1']);
...
// 9. Hết hạn thời gian → trả phần đã quét, đánh dấu timedOut, không treo.
r = await findSources(root, ['Higg 33'], { budgetMs: 1, concurrency: 1 });
assert.strictEqual(r.ok, true);
